"""Daemon control sub-commands for ``claude-fleet daemon ...``.

daemon-stop    — send SIGTERM to a running daemon
daemon-status  — check liveness of a running daemon

Note: 'claude-fleetd start' lives in orchestrator/__main__.py (the
claude-fleetd console script entrypoint), not here.
"""
...
DEFAULT_DAEMON_LOG_CANDIDATES: tuple[Path, ...] = (
    Path("logs/pm2-claude-fleet-daemon-out.log"),
    Path("logs/claude-fleet-daemon.log"),
...
def _pm2_jlist(pm2_name: str | None = None) -> dict[str, Any] | None:
    ...
    if not isinstance(data, list):
        return None
    names = [pm2_name] if pm2_name else ["claude-fleet-daemon", "claude-fleetd"]
    for name in names:
        for entry in data:
...
def daemon_status_cmd(
    ...
    1 when DEAD.
    """
    db_path = Path(ctx.obj["db"]) if ctx.obj and "db" in ctx.obj else Path("./claude-fleet.db")
