...
- :class:`EnvVarSelector` — round-robins over a static list, defaulting to
  ``CLAUDE_FLEET_PROFILES`` env var. No external binary required.
...
class EnvVarSelector(ProfileSelector):
    ...
    ``CLAUDE_FLEET_PROFILES`` from the environment at instantiation time.

    Parameters
    ----------
    profiles:
        Explicit list of profile names. If ``None`` or empty, falls back to
        ``os.environ["CLAUDE_FLEET_PROFILES"].split(",")`` at construction.
    ...
    def __init__(self, profiles: list[str] | None = None) -> None:
        if not profiles:
            raw = os.environ.get("CLAUDE_FLEET_PROFILES", "").strip()
    ...
    async def pick(self) -> ProfileInfo:
        ...
        if not self._profiles:
            raise ProfileListEmpty(
                ...
                "Set CLAUDE_FLEET_PROFILES=<comma-list> or pass profiles= explicitly."
            )
