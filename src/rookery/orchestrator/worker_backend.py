"""WorkerBackend: spawn parcel workers via the :mod:`rookery.platform` API.
...
Paired with :class:`rookery.orchestrator.land_backend.LandBackend` —
...
def _resolve_claude_config_dir(profile: str | None) -> str | None:
    ...
    5. If profile is None or not found: fall back to the user's default
       ``~/.claude/`` dir.
    """
    explicit = os.environ.get("ROOKERY_CONFIG_DIR")
    if explicit and Path(explicit).is_dir():
        return explicit

    env_profile = os.environ.get("ROOKERY_PROFILE")
...
def _discover_claude_bin_dir() -> str | None:
    ...
    1. ``ROOKERY_BIN_DIR`` env var (operator override, e.g. set in
    ...
    """
    explicit = os.environ.get("ROOKERY_BIN_DIR")
    if explicit and Path(explicit).is_dir():
        return explicit
...
class WorkerBackend(OrchestratorBackend):
    ...
    claude_profile:
        ...
        Env vars ``ROOKERY_CONFIG_DIR`` (absolute path) and
        ``ROOKERY_PROFILE`` (profile name) override this per
        daemon instance. Env var ``ROOKERY_PROFILES`` (comma-list)
    ...
    def __init__(
        ...
        *,
        ...
        # spawn.  If None but ROOKERY_PROFILES is set in the environment,
        ...
        if profile_selector is not None:
            ...
            profiles_env = os.environ.get("ROOKERY_PROFILES", "").strip()
            if profiles_env:
        ...
        # ROOKERY_PROFILES was empty at construction time).
        profiles_env_legacy = os.environ.get("ROOKERY_PROFILES", "").strip()
    ...
    def _next_profile(self) -> str | None:
        ...
        1. ``claude-lb pick --require-ok`` when the CLI is installed.
           ...
           naive round-robin).
        2. Round-robin across ``ROOKERY_PROFILES`` when set — fallback
    ...
    def _build_env(self, profile_override: str | None = None) -> dict[str, str]:
        ...
        - Strips any ``CLAUDE_CODE_OAUTH_TOKEN`` inherited from the daemon's
          ...
          inside claude CLI. (See ``ROOKERY_PROFILE`` / ``ROOKERY_CONFIG_DIR``
