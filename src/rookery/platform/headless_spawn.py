...
* Export ``ROOKERY_WORKTREE_ID`` so the child uses our canonical id
...
from rookery.platform.worktree_id import resolve_worktree_id
...
def spawn_headless_claude(
    *,
    ...
    # Propagate (or derive) ROOKERY_WORKTREE_ID so the child sees the
    # same canonical id as the parent launch surface.
    base_env.setdefault("ROOKERY_WORKTREE_ID", resolve_worktree_id(worktree))
    if extra_env:
