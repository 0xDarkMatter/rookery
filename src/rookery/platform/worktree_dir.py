...
Every ``rookery`` launch surface needs the same pair of operations:

* :func:`resolve_worktree_dir` — map a parcel id to its expected on-disk
  location (``<repo>/../rookery-worktrees/<id>``), **without** creating it.
...
WORKTREES_DIRNAME = "rookery-worktrees"
...
def repo_paths(repo_root: Path) -> RepoPaths:
    """Return the canonical ``rookery-worktrees`` layout rooted at *repo_root*.
