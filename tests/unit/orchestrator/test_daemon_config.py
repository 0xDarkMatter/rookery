"""Regression tests for daemon db_path resolution from claude-fleet.yaml.

Covers the bug where ``claude-fleetd start`` ignored the yaml ``db_path``
...
Resolution order under test (mirrors __main__.start_cmd):
    1. explicit ``--db`` flag > yaml ``db_path`` > hardcoded fallback ./claude-fleet.db
"""
...
def test_yaml_db_path_is_respected(tmp_path: Path) -> None:
    ...
    config_file = tmp_path / "claude-fleet.yaml"
...
def test_missing_config_falls_back_to_default(tmp_path: Path) -> None:
    """When the config file doesn't exist, fall back to ./claude-fleet.db."""
    missing = tmp_path / "no-such-file.yaml"

    resolved = _resolve_db_path(missing)

    assert resolved == Path("./claude-fleet.db"), (
        f"Expected ./claude-fleet.db fallback but got {resolved}."
    )


def test_yaml_without_db_path_uses_default(tmp_path: Path) -> None:
    """A yaml that omits db_path should still fall back to ./claude-fleet.db."""
    config_file = tmp_path / "claude-fleet.yaml"
...
def test_old_default_data_dir_is_never_used(tmp_path: Path) -> None:
    ...
    config_file = tmp_path / "claude-fleet.yaml"
    config_file.write_text("db_path: ./claude-fleet.db\n", encoding="utf-8")
...
def test_load_config_returns_yaml_db_path_directly(tmp_path: Path) -> None:
    ...
    from claude_fleet.orchestrator.config import load_config
